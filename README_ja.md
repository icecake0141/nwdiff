...
- **エラーログ機能:**  
  すべての操作、エラー、システムイベントを追跡する包括的なログシステム。
  - ログは `logs` ディレクトリに保存されます
  - Web インターフェース `/logs` でログを表示できます
  - API `/api/logs` を通じてログにアクセスできます

- **設定バックアップ:**  
  キャプチャされた設定の自動バックアップローテーション。
  - 既存のキャプチャを上書きする前にバックアップが作成されます
  - ファイルごとに最新の10個のバックアップが保持されます
  - バックアップはタイムスタンプ付きで `backup` ディレクトリに保存されます

- **エクスポート機能:**  
  自動処理や外部統合のため、差分結果を JSON としてエクスポート。
  - `/api/export/<hostname>` を通じてアクセス
  - すべてのコマンド結果、タイムスタンプ、差分ステータスが含まれます
  - CI/CD パイプラインや監視システムに便利です

- **検索とフィルタ:**  
  検索およびフィルタ機能を備えた強化された UI。
  - 名前または IP アドレスでホストを検索
  - 差分ステータスでフィルタ（変更検出、同一、ファイル未検出）
